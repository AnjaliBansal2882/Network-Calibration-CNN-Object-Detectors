...
def reliability_plot(bin_dict, prec, loss, modl):
    ...
    difference_heights = [prec[i] - heights[i] for i in range(len(heights))]
    ...
    ax.set_ylim(0, 1.02)
    # Display the plot
    file_path = f"/home/Anjali/Desktop/Anjali_dev/calib_Plots/DECE_Plots/cf_0.25/RP/DECE_{loss}_{modl}.png"
...
def confidence_histogram(conf_dict, prec, loss, modl):
    ...
    plt.plot([avg_precision, avg_precision], [0, max(heights) + 2000], linestyle = "--", color = "red", label = f"Avg prec = {avg_precision:.2f}")
    ...
    file_path = f"/home/Anjali/Desktop/Anjali_dev/calib_Plots/DECE_Plots/cf_0.25/CH/DECE_{loss}_{modl}.png"
    plt.savefig(file_path)
...
for loss in losses:
    for modl in models:
        ...
        if os.path.exists(model_path):
            ...
            reliability_plot(avg_conf_dict, prec, loss, modl)
            confidence_histogram(conf_dict, prec, loss, modl)
        else:
